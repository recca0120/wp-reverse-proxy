...
### 方式三：WordPress 後台管理介面

外掛提供圖形化介面，可在 WordPress 後台直接管理路由，無需編寫程式碼或設定檔。
...
1. **簡易模式** - 下拉選單選擇中介層，填寫對應參數
2. **JSON 模式** - 直接編輯 JSON，適合進階設定
...
| □ | 路徑         | 目標 URL                | 操作 |
+---+--------------+------------------------+------+
| ✓ | /api/*       | https://api.example.com| 編輯 |
| ✓ | /legacy/*    | https://old.example.com| 編輯 |
| ○ | /test/*      | https://test.local     | 編輯 |
+--------------------------------------------------+
```
